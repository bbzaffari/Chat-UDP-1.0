        ...
        if (FD_ISSET(sock8, &readfds)) {
            ...
            //Na porta 50008 ocorre troca de mensagens------------------------------------------------------------------------------------------------------------------------------*/
                ...
                if(!(is_client_in_list(client_ip))){//Nao esta na lista
                    client_address.sin_port = htons(50005); //coigo de controle ouve algum erro
                    len = sizeof(client_address);//Mata sender
                    sendto(sock8, "1", 1, 0, (struct sockaddr*)&client_address, len); //5<-7 1=Cliente nao esta na lista
                    
                    client_address.sin_port = htons(50004); // Responder ao cliente na porta 5 50005
                    len = sizeof(client_address);//Mata reciver
                    sendto(sock8, "0", 2, 0, (struct sockaddr*)&client_address, len);//Codigo de controle para avisar q sabe q o cliente saiu  4<-8
                }
                else{
                    ...
                    if(strcmp(cod, "/sai") == 0){//SAI --------------------------------------------------------------------------------------------------------
                        //Criar funcao para deixar cliente suspenso;
                        printf("--------------->Suspensao de cliente!\n");
                        print_client_list();
                        
                        client_address.sin_port = htons(50005); // Responder ao cliente na porta 5 50005
                        len = sizeof(client_address);//Mata sender
                        sendto(sock8, "2", 1, 0, (struct sockaddr*)&client_address, len);//Codigo de controle para avisar q sabe q o cliente saiu  5<-8
                    
                        client_address.sin_port = htons(50004); // Responder ao cliente na porta 5 50005
                        len = sizeof(client_address);//Mata reciver
                        sendto(sock8, "0", 2, 0, (struct sockaddr*)&client_address, len);//Codigo de controle para avisar q sabe q o cliente saiu  4<-8
                    }
                    if(strcmp(cod, "/out") == 0){//OUT  --------------------------------------------------------------------------------------------------------
                        remove_client(client_ip);
                        printf("--------------->Remoção de cliente!\n");
                        print_client_list();
                        client_address.sin_port = htons(50005); // Responder ao cliente na porta 5 50005
                        len = sizeof(client_address);//Mata sender
                        sendto(sock8, "4", 1, 0, (struct sockaddr*)&client_address, len);//Codigo de controle para avisar q sabe q o cliente saiu  5<-8
                    ...
                    if((strcmp(cod, "/arq")== 0)){//ARQ  -------------------------------------------------------------------------------------------------------------
                        ...
                        //Envia Arquivo--------------------------------------------------------------------------------------------------------------------
                        send_to_all_clients(sock8, "!", 50004, 2);// Avisa que vai transmitir um arquivo 4<-8
