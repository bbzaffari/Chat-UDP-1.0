...
#define SERV_IP  "192.168.56.102"
//#define SERV_IP "172.20.10.12"
...
int main(){
    ...
    server_address7.sin_addr.s_addr = inet_addr(SERV_IP);

    // Configurar endereço do servidor para porta 50008
    server_address8.sin_family = AF_INET;
    server_address8.sin_port = htons(50008);
    server_address8.sin_addr.s_addr = inet_addr(SERV_IP);
    /*------------------------------------------------------------------------------------------------------------------------------------------------------------*/
    // verifica se es processo ou filho
    //int l;
    //printf("to aqui %d\n", l);
    //sleep(1);
    while (1)
    {           
        ...
        if((receiver != 0) && (sender != 0)){//Executa Pai----------------------------------------------------------------------------------------------------------------------------------
            while (conectado == 0 ){
                if(opa3 ==0)
                {
                    ...
                    { // LIN -----------------------------------------------------------------------------------------------------
                        ...
                        if (x > 0)
                        {
                            ...
                            { // talvez tirar daqui
                                ...
                                conectado = 1; // Anteriormente/Posteriormente era/vai ser para entrar da thread/processo sender e reciver                                
                            }
       ...
       // Termino do codigo Pai ------------------------------------------------------------------------------------------------------------------------------
        
        /*---------------------------------------------------------------------------------------------------------------------------------------------------*/
        else if(receiver == 0){//Codigo do receiver----------------------------------------------------------------------------------------------------------
            //sleep(3);
            while(1){
            //printf("receiver\n"); //debug
            len = sizeof(server_address8);
            memset(recebeporta4, 0, sizeof(recebeporta4));
            x = recvfrom(sock4, recebeporta4, sizeof(recebeporta4) - 1, 0, (struct sockaddr *)&server_address8, &len);
            if((x)>0){// aguarda receber o nome 4<-8
                recebeporta4[x] = '\0';
                if(recebeporta4[0]=='!'){
                    len = sizeof(server_address8);
                    memset(recebeporta4, 0, sizeof(recebeporta4));
                    ...
                    if (x == -1)perror("Erro ao receber dados");
                    else{
                        recebeporta4[x] = '\0'; // Adicionar o null-terminator manualmente
                        x = -2;
                ...
                // Recebimento de Arquivo-----------------------------------------------------------------------------------------------------------Fim
                else if(recebeporta4[0]=='0') exit(0); // Mata receiver //* espelho
        ...
        // Codigo do sender: ----------------------------------------------------------------------------------------------------------------------------------
            ...
            while (1){
                ...
                if (segmenta_geral(envioporta8, txt, cod, nome, mensagem)){ // /lin /out /sai /msg /mpv----------------------------------------------------------------------------------------------------------
                    if (strcmp(cod, "/lin") == 0){// LIN -----------------------------------------------------------------------------------------------------
                        ...
                        x = sendto(sock4, envioporta8, sizeof(envioporta8), 0, (struct sockaddr *)&server_address8, sizeof(server_address8)); // 4->8
                        if (x > 0)
                        {
                            len = sizeof(server_address8);
            ...
            if (conectado == 1 && sender> 0 && receiver>0){
                sender = fork();
                if(sender > 0)receiver = fork();
            }
            if (conectado == 1 && sender> 0 && receiver>0){
